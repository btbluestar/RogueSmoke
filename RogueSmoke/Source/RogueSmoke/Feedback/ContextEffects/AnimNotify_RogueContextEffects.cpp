...
#include "Feedback/ContextEffects/AnimNotify_RogueContextEffects.h"
#include "Feedback/ContextEffects/RogueContextEffectsLibrary.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimNotify_RogueContextEffects)



UAnimNotify_RogueContextEffects::UAnimNotify_RogueContextEffects()
{
}

void UAnimNotify_RogueContextEffects::PostLoad()
{
	Super::PostLoad();
}

#if WITH_EDITOR
void UAnimNotify_RogueContextEffects::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
}
#endif

FString UAnimNotify_RogueContextEffects::GetNotifyName_Implementation() const
{
...
void UAnimNotify_RogueContextEffects::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	...
	if (MeshComp)
	{
		...
		if (AActor* OwningActor = MeshComp->GetOwner())
		{
			...
			TArray<UObject*> RogueContextEffectImplementingObjects;

			// Determine if the Owning Actor is one of the Objects that implements the Context Effects Interface
			if (OwningActor->Implements<URogueContextEffectsInterface>())
			{
				// If so, add it to the Array
				RogueContextEffectImplementingObjects.Add(OwningActor);
			}
			...
			for (const auto Component : OwningActor->GetComponents())
			{
				if (Component)
				{
					...
					if (Component->Implements<URogueContextEffectsInterface>())
					{
						RogueContextEffectImplementingObjects.Add(Component);
					}
				}
			}

			// Cycle through all objects implementing the Context Effect Interface
			for (UObject* RogueContextEffectImplementingObject : RogueContextEffectImplementingObjects)
			{
				if (RogueContextEffectImplementingObject)
				{
					// If the object is still valid, Execute the AnimMotionEffect Event on it, passing in relevant data
					IRogueContextEffectsInterface::Execute_AnimMotionEffect(RogueContextEffectImplementingObject,
			...
			if (bPreviewInEditor)
			{
				...
				if (World && World->WorldType == EWorldType::EditorPreview)
				{
					...
					{
						// Check if it is in fact a URogueContextEffectsLibrary type
						if (URogueContextEffectsLibrary* EffectLibrary = Cast<URogueContextEffectsLibrary>(EffectsLibrariesObj))
						{
...
#if WITH_EDITOR
void UAnimNotify_RogueContextEffects::ValidateAssociatedAssets()
{
	Super::ValidateAssociatedAssets();
}

void UAnimNotify_RogueContextEffects::SetParameters(FGameplayTag EffectIn, FVector LocationOffsetIn, FRotator RotationOffsetIn,
	FRogueContextEffectAnimNotifyVFXSettings VFXPropertiesIn, FRogueContextEffectAnimNotifyAudioSettings AudioPropertiesIn,
	bool bAttachedIn, FName SocketNameIn, bool bPerformTraceIn, FRogueContextEffectAnimNotifyTraceSettings TracePropertiesIn)
{
	Effect = EffectIn;
