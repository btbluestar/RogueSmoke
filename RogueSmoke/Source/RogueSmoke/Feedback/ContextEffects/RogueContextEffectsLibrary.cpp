...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RogueContextEffectsLibrary)


void URogueContextEffectsLibrary::GetEffects(const FGameplayTag Effect, const FGameplayTagContainer Context,
...
void URogueContextEffectsLibrary::LoadEffects()
{
...
EContextEffectsLibraryLoadState URogueContextEffectsLibrary::GetContextEffectsLibraryLoadState()
{
	// Return current Load State
	return EffectsLoadState;
}

void URogueContextEffectsLibrary::LoadEffectsInternal()
{
	// TODO Add Async Loading for Libraries

	// Copy data for async load
	TArray<FRogueContextEffects> LocalContextEffects = ContextEffects;

	// Prepare Active Context Effects Array
	TArray<URogueActiveContextEffects*> ActiveContextEffectsArray;

	// Loop through Context Effects
	for (const FRogueContextEffects& ContextEffect : LocalContextEffects)
	{
		// Make sure Tags are Valid
		if (ContextEffect.EffectTag.IsValid() && ContextEffect.Context.IsValid())
		{
			// Create new Active Context Effect
			URogueActiveContextEffects* NewActiveContextEffects = NewObject<URogueActiveContextEffects>(this);
	...
	this->RogueContextEffectLibraryLoadingComplete(ActiveContextEffectsArray);
}

void URogueContextEffectsLibrary::RogueContextEffectLibraryLoadingComplete(
	TArray<URogueActiveContextEffects*> RogueActiveContextEffects)
{
	// Flag data as loaded
	...
	ActiveContextEffects.Append(RogueActiveContextEffects);
}
