...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RogueContextEffectsSubsystem)
...
void URogueContextEffectsSubsystem::SpawnContextEffects(
	...
	, float AudioPitch)
{
	...
	if (TObjectPtr<URogueContextEffectsSet>* EffectsLibrariesSetPtr = ActiveActorEffectsMap.Find(SpawningActor))
	{
		// Validate the pointers from the Map Find
		if (URogueContextEffectsSet* EffectsLibraries = *EffectsLibrariesSetPtr)
		{
			...
			for (URogueContextEffectsLibrary* EffectLibrary : EffectsLibraries->RogueContextEffectsLibraries)
			{
...
bool URogueContextEffectsSubsystem::GetContextFromSurfaceType(
	TEnumAsByte<EPhysicalSurface> PhysicalSurface, FGameplayTag& Context)
{
	// Get Project Settings
	if (const URogueContextEffectsSettings* ProjectSettings = GetDefault<URogueContextEffectsSettings>())
	{
...
void URogueContextEffectsSubsystem::LoadAndAddContextEffectsLibraries(AActor* OwningActor,
	TSet<TSoftObjectPtr<URogueContextEffectsLibrary>> ContextEffectsLibraries)
{
	...
	URogueContextEffectsSet* EffectsLibrariesSet = NewObject<URogueContextEffectsSet>(this);

	// Cycle through Libraries getting Soft Obj Refs
	for (const TSoftObjectPtr<URogueContextEffectsLibrary>& ContextEffectSoftObj : ContextEffectsLibraries)
	{
		// Load Library Assets from Soft Obj refs
		// TODO Support Async Loading of Asset Data
		if (URogueContextEffectsLibrary* EffectsLibrary = ContextEffectSoftObj.LoadSynchronous())
		{
			// Call load on valid Libraries
			EffectsLibrary->LoadEffects();

			// Add new library to Set
			EffectsLibrariesSet->RogueContextEffectsLibraries.Add(EffectsLibrary);
		}
	}
...
void URogueContextEffectsSubsystem::UnloadAndRemoveContextEffectsLibraries(AActor* OwningActor)
{
